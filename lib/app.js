...
const { getWeatherData, getReviewData } = require('./fetch-utils.js');
...
app.get('./location', async (req, res) => {
  try {
    ...
    const returnData = {
      ...
      longitutde: apiData[0].longitutde,
    };
    res.json(returnData);
  } catch (e) {
    res.status(500).json({ error: e.message });
...
app.get('./weather', async (req, res) =>{
  try {
    ...
    const lon = req.query.longitutde;
...
app.get('./reviews', async (req, res) =>{
  try {
    const lat = req.query.latitude;
    const lon = req.query.longitutde;
    const data = await getReviewData(lat, lon);
    res.json(data);
  } catch (e) {
    res.status(500).json ({ error: e.message });
  }
